...
describe("buildUrl", () => {
  ...
  it("omits ? when there is no query", () => {
    expect(buildUrl("http://x:4000", "/trips/me")).toBe(
      "http://x:4000/trips/me",
    );
  });
});
...
describe("errorMessage", () => {
  it("joins Nest validation message arrays", () => {
    expect(errorMessage({ message: ["a is bad", "b is bad"] }, 400)).toBe(
      "a is bad, b is bad",
    );
  });
...
describe("createHttp", () => {
  it("sends JSON bodies with merged headers and parses JSON", async () => {
    ...
    await expect(http.post("/buyers", { name: "A" })).resolves.toEqual({
      id: "1",
    });
  ...
  it("throws ApiError with status and Nest message", async () => {
    const http = createHttp({
      baseUrl: "http://x",
      fetch: async () => json(400, { message: "bad kilo" }),
    });
  ...
  it("calls onUnauthorized on 401 and still throws", async () => {
    ...
    const http = createHttp({
      baseUrl: "http://x",
      onUnauthorized,
      fetch: async () => json(401, { message: "Unauthorized" }),
    });
