...
describe("refreshTokens", () => {
  ...
  it("passes through a grace-window answer with no new refresh token", async () => {
    const r = await refreshTokens("http://x", "r-grace", {}, async () =>
      res(201, { accessToken: "a3", refreshToken: null }),
    );
    expect(r).toEqual({
      status: "ok",
      tokens: { accessToken: "a3", refreshToken: null },
    });
  });
  it("is 'invalid' only when the backend rejects the token", async () => {
    expect(
      await refreshTokens("http://x", "r-401", {}, async () => res(401)),
    ).toEqual({ status: "invalid" });
    expect(
      await refreshTokens("http://x", "r-400", {}, async () => res(400)),
    ).toEqual({ status: "invalid" });
  });
  it("is 'unavailable' when the backend is down, erroring or throttling (keep the session)", async () => {
    expect(
      await refreshTokens("http://x", "r-500", {}, async () => res(500)),
    ).toEqual({ status: "unavailable" });
    expect(
      await refreshTokens("http://x", "r-429", {}, async () => res(429)),
    ).toEqual({ status: "unavailable" });
    expect(
      await refreshTokens("http://x", "r-down", {}, async () => {
