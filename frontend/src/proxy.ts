...
export async function proxy(request: NextRequest) {
  ...
  const user = refresh
    ? parseUser(request.cookies.get(COOKIE.user)?.value)
    : null;
  ...
  if (
    !user ||
    !refresh ||
    !isExpiring(request.cookies.get(COOKIE.access)?.value)
  )
  ...
  const result = await refreshTokens(
    API_URL,
    refresh,
    backendHeaders(request.headers, INTERNAL_PROXY_SECRET),
  );
  ...
  if (changed && !canAccess(changed.role, pathname)) {
    const res = NextResponse.redirect(
      new URL(homeFor(changed.role), request.url),
    );
...
export const config = {
  matcher: [
    "/((?!api|serwist|manifest.webmanifest|_next/static|_next/image|favicon.ico|.*\\.(?:svg|png|jpg|jpeg|gif|webp|ico)$).*)",
  ],
};
